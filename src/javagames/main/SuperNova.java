...
public class SuperNova extends SimpleFramework {
	...
	private OneShotEvent bossDeath;
	private byte[] bossDeathBytes;
    ...
    protected void initialize() {
        ...
        initBackground();
        initPlayer();
        InputStream in = ResourceLoader.load(SuperNova.class,
        		"./assets/sounds/ambient.wav","/sounds/ambient.wav");
		ambientBytes = readBytes(in);
		in = ResourceLoader.load(SuperNova.class,
				"./assets/sounds/bossDeath.wav", "/sounds/bossDeath.wav");
		bossDeathBytes = readBytes(in);
		in = ResourceLoader.load(SuperNova.class,
		...
		shutDownClips();
		bossDeath = new OneShotEvent(new BlockingClip(bossDeathBytes));
		bossDeath.initialize();
    ...
    protected void reset(){
    	...
    	if(level == 2){
    		Boss tmpBoss = new Boss(new Vector2f(9.75f, -53.5f));
    		tmpBoss.health = 15;
    		enemies.add(tmpBoss);
    	}
        //preempt deadEnemies for fast object manipulation
    ...
    protected void disableCursor() {
	      ...
	      if(image== null)
	    	  image = tk.createImage("/actors/cursor.png");
    ...
    protected void updateEnemies(float delta){
        for(int i=0;i<enemies.size();++i){
        	...
        	if(enemy.inRange){
        		//Enemy shooting
				if(bossFight && enemy.health<15 && level==1){
    ...
    protected void render(Graphics g) {
        ...
        super.render(g);
    		...
    		String str = "";
    		String credits = "Credits: Miles Ring, Samuel Groenjes, and Rodrigo Sanchez ";
    		if(player.health <= 0){
    			str = "You are dead.";
    		}
    		else if(level == 2 && enemies.isEmpty()){
    			str = "Zelroth has been defeated!!!";
    			endGame = true;
    			shutDownClips();
    			victory.fire();
    		}
    		else if(level == 2){
    			str = "Zelroth has escaped to his lair!";
    		}	
    		else{
			...
			g.drawString(str, appWidth/2, appHeight/2);
			if(endGame){
				g.drawString(credits, appWidth/2-400, appHeight/2+100);
			}
